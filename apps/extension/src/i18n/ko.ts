import type { MessageDictionary } from "./en";

export const ko: MessageDictionary = {
  "status.loading": "상태 확인 중…",
  "status.wake": "거래소 화면을 띄우면 깨워줘.",
  "status.saved": "설정을 저장했어.",
  "status.historyCleared": "Paper 기록을 지웠어.",
  "status.learningReset": "학습을 초기화했어.",
  "status.dataReset": "로컬 Fly 데이터를 모두 지웠어.",
  "bubble.login": "로그인하면 포트폴리오도 볼 수 있어.",
  "bubble.marketUnavailable": "시세 데이터를 못 받아서 쉬고 있어.",
  "bubble.brainUnavailable": "MaleCNS가 끊겨서 쉬고 있어.",
  "bubble.approachBuy": "매수 버튼 쪽에 끌리고 있어.",
  "bubble.approachSell": "매도 버튼 쪽을 기웃거리는 중.",
  "bubble.scanAssets": "오른쪽 종목 리스트를 훑어보는 중.",
  "bubble.observeChart": "차트를 가만히 살펴보는 중.",
  "bubble.explore": "화면 여기저기를 돌아다니는 중.",
  "bubble.panic": "변동이 커서 조금 도망가는 중.",
  "popup.mode": "모드",
  "popup.trading": "거래",
  "popup.brain": "뇌",
  "popup.maxCapital": "최대 거래 자본",
  "popup.startingCapital": "Paper 시작 자본",
  "popup.save": "설정 저장",
  "popup.language": "언어",
  "popup.languageAuto": "자동",
  "popup.languageKo": "한국어",
  "popup.languageEn": "English",
  "popup.learning": "학습",
  "popup.learningEnable": "내 Paper 거래 기록으로 학습하기",
  "popup.learningSamples": "학습 샘플",
  "popup.buyThreshold": "매수 임계값",
  "popup.sellThreshold": "매도 임계값",
  "popup.resetLearning": "학습 초기화",
  "popup.learningPrivacy":
    "학습은 이 기기에서만 실행됩니다. 거래 관찰 데이터는 외부로 전송되지 않습니다.",
  "popup.globalLearning": "글로벌 학습",
  "popup.contributeLearning": "익명 Paper 학습 데이터 제공",
  "popup.contributePrivacy":
    "제공된 데이터는 모든 사용자가 동일하게 사용하는 공통 calibration 개선에 활용됩니다. 계좌 인증정보나 로그인 정보는 전송되지 않습니다.",
  "popup.presetVersion": "현재 공통 보정",
  "popup.queuedObservations": "대기 중 관찰",
  "popup.localLearning": "로컬 학습",
  "popup.localObservations": "관측",
  "popup.localOutcomes": "해결된 미래 결과",
  "popup.closedTrades": "종료된 Paper",
  "popup.lastSync": "마지막 동기화",
  "popup.clearQueue": "기여 큐 비우기",
  "popup.syncLearning": "지금 동기화",
  "popup.rollbackPreset": "번들 프리셋으로 롤백",
  "popup.sameQuality":
    "같은 Fly 버전과 Global Calibration 버전을 사용하는 사용자는 동일한 calibration 품질로 시작합니다.",
  "popup.oneFlyShared": "하나의 Fly, 함께 쌓는 학습 데이터",
  "popup.oneFlySharedBody":
    "모든 사용자는 동일하게 공개된 Global Calibration을 사용합니다. 커뮤니티의 익명 Paper 결과는 다음 공통 보정 버전을 평가하는 데 활용됩니다.",
  "popup.consentContribute": "익명 Paper 결과 제공",
  "popup.consentLocalOnly": "Paper 학습 데이터를 이 기기에만 저장",
  "popup.paperObservations": "Paper 관찰 수",
  "popup.lastCalibration": "마지막 보정",
  "popup.unavailableStat": "—",
  "onboarding.title": "함께 Fly를 개선해 주세요",
  "onboarding.body":
    "익명화된 Paper 거래 결과를 제공하면 모든 사용자가 동일하게 사용하는 Fly의 공통 보정값 개선에 활용됩니다.",
  "onboarding.moreEvidence":
    "Fly를 사용하는 사람이 많아지고 Paper 결과가 많이 쌓일수록, 다음 공통 보정값을 평가하고 개선할 수 있는 근거 데이터가 더 많아집니다.",
  "onboarding.notRetrain":
    "MaleCNS 초파리 connectome 자체를 재학습하거나 수정하는 것은 아닙니다.",
  "onboarding.neverCollect": "의도적으로 수집하지 않는 정보:",
  "onboarding.neverCredentials": "거래소 로그인 정보",
  "onboarding.neverAccount": "계좌번호",
  "onboarding.neverHoldings": "실제 보유자산 및 잔고",
  "onboarding.neverLive": "실제 돈으로 실행한 거래",
  "onboarding.neverCookies": "쿠키 및 인증 토큰",
  "onboarding.neverSymbols": "원본 주식/코인 종목명",
  "onboarding.neverEmail": "이메일 및 개인 식별정보",
  "onboarding.choose": "Fly 사용 방식을 선택해 주세요.",
  "onboarding.continue": "계속",
  "popup.performance": "초파리 성과",
  "popup.experimentalPerformance": "실험용 Paper 성과",
  "popup.rangeAll": "전체",
  "popup.range7d": "7일",
  "popup.range30d": "30일",
  "popup.byBrainMode": "뇌 모드별",
  "popup.diagnostics": "셀렉터 / 페이지 진단",
  "popup.privacy": "개인정보",
  "popup.privacyBody":
    "비밀번호·OTP·쿠키·세션 토큰은 읽거나 저장하지 않습니다.",
  "popup.clearPaper": "Paper 기록 삭제",
  "popup.resetAll": "로컬 Fly 데이터 전체 초기화",
  "popup.fineprint":
    "BUY 근처 이동 ≠ 수익 예측. Experimental MaleCNS-constrained neural response only. 과거 Paper 성과가 미래 수익을 보장하지 않습니다.",
  "perf.totalReturn": "총 수익률",
  "perf.realizedPnl": "실현 손익",
  "perf.trades": "거래 수",
  "perf.winRate": "승률",
  "perf.avgWin": "평균 이익",
  "perf.avgLoss": "평균 손실",
  "perf.profitFactor": "Profit Factor",
  "perf.maxDrawdown": "최대 낙폭",
  "perf.bestTrade": "최고 거래",
  "perf.worstTrade": "최악 거래",
  "confirm.resetLearning":
    "로컬 학습 관찰과 calibration을 초기화할까요? Paper 기록은 유지됩니다.",
  "confirm.clearPaper": "Paper 거래와 사이클을 지울까요?",
  "confirm.resetAll":
    "로컬 Fly 데이터 전부(학습·Paper·설정 기본값)를 초기화할까요?",
  "popup.exploration": "자율 탐색",
  "popup.explorationOn": "자율 시장 탐색",
  "popup.explorationSpeed": "탐색 속도",
  "popup.speedSlow": "느리게",
  "popup.speedNormal": "보통",
  "popup.speedFast": "빠르게",
  "popup.visibleControl": "화면 조작 보이기",
  "popup.activityHud": "Fly 활동 HUD",
  "popup.pauseExploration": "탐색 일시정지 / 화면 조종 중단",
  "popup.paperAutoTrade": "Paper 자동거래",
  "popup.tradeNotifications": "거래 알림",
  "popup.flyOverlay": "거래소 Fly 오버레이",
  "popup.advancedExploration": "고급 (deprecated 자율 탐색)",
  "fly.ux.OBSERVING": "관찰 중",
  "fly.ux.WATCHING": "주시 중",
  "fly.ux.BUYING": "매수 중",
  "fly.ux.HOLDING": "보유 중",
  "fly.ux.SELLING": "매도 중",
  "fly.ux.RESTING": "쉬는 중",
  "overlay.virtualCapital": "가상 자본",
  "overlay.cumulative": "누적",
  "overlay.start": "시작",
  "overlay.current": "현재",
  "overlay.position": "포지션",
  "overlay.unrealized": "미실현",
  "overlay.positionNone": "없음",
  "overlay.positionHolding": "보유",
  "toast.paperBuyTitle": "Fly가 {symbol}를 매수했어요",
  "toast.paperBuyBody": "가상 매수: {value} · 가격: {price}",
  "toast.paperSellTitle": "Fly가 {symbol}를 매도했어요",
  "toast.paperSellBody":
    "이번 거래: {return} · 손익: {pnl} · 누적: {cumulative}",
};

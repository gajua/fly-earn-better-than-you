...
test.describe("Binance usable Paper path", () => {
  test.skip(!enabled, "Set RUN_EXTENSION_E2E=1 with E2E_TEST_MODE=1 build");

  test("extension Paper BUY/SELL on public Binance with real-connectome", async () => {
    ...
    try {
      ...
      expect(diagnostics?.brokerId).toBe("binance");
      ...
      const reloadedClosed =
        (
          afterReload?.exposure as
            { cycles?: { status?: string }[] } | undefined
        )?.cycles?.filter((cycle) => cycle.status === "closed").length ?? 0;
      expect(reloadedClosed).toBeGreaterThanOrEqual(1);
      expect(
        (afterReload?.paper as { totalTrades?: number })?.totalTrades,
      ).toBeGreaterThanOrEqual(2);

      try {
        await harness.serviceWorker.evaluate(() => chrome.runtime.reload());
        await harness.context.waitForEvent("serviceworker", {
          timeout: 20_000,
        });
        await page.waitForTimeout(3_000);
        const afterSw = await readPerformance(popup);
        expect(
          (afterSw?.paper as { totalTrades?: number })?.totalTrades,
        ).toBeGreaterThanOrEqual(2);
      } catch {
        // best-effort SW restart
      }
